...
add("model.textEncoder.mlx", {
    'en': 'Qwen3-VL-32B in bfloat16 — H3 reads its 50th-layer hidden states. The largest single download, and currently the only text encoder the MLX pipeline can load. At about 34 GB resident it, not the transformer, decides whether this app runs on a given Mac: a fixed cost no quantization reduces, against which a smaller transformer saves at most 29 GB. Weights alone are not the test, though — see the Requirements tab for what a machine actually needs. It installs beside the VAEs in FL2VA/.',
    'zh-Hant': 'bfloat16 版的 Qwen3-VL-32B——H3 讀取其第 50 層的隱藏狀態。單一檔案中最大的下載項目，目前也是 MLX pipeline 唯一能載入的文字編碼器。常駐約 34 GB，因此決定本 App 能否在某台 Mac 上執行的是它而非 transformer：這是量化也減不掉的固定成本，換用較小的 transformer 最多只省 29 GB。不過單看權重並非判準——實際需要多少，請看「系統需求」分頁。會與 VAE 一同安裝在 FL2VA/ 之下。',
    'zh-Hans': 'bfloat16 版的 Qwen3-VL-32B——H3 读取其第 50 层的隐藏状态。单个文件中最大的下载项，目前也是 MLX pipeline 唯一能加载的文本编码器。常驻约 34 GB，因此决定本 App 能否在某台 Mac 上运行的是它而非 transformer：这是量化也减不掉的固定成本，换用较小的 transformer 最多只省 29 GB。不过单看权重并非判准——实际需要多少，请看“系统需求”分页。会与 VAE 一同安装在 FL2VA/ 之下。',
    'de': 'Qwen3-VL-32B in bfloat16 – H3 liest dessen Hidden States aus Schicht 50. Der größte Einzeldownload und derzeit der einzige Text-Encoder, den die MLX-Pipeline laden kann. Mit rund 34 GB resident entscheidet er, nicht der Transformer, ob die App auf einem Mac läuft: ein fester Posten, den keine Quantisierung senkt, und ein kleinerer Transformer spart höchstens 29 GB dagegen. Die Gewichte allein sind aber nicht der Maßstab — was eine Maschine wirklich braucht, steht im Tab „Anforderungen“. Wird neben den VAEs unter FL2VA/ installiert.',
    'ar': '\u200fQwen3-VL-32B بدقة bfloat16 — يقرأ H3 الحالات المخفية من طبقته الخمسين. أكبر تنزيل مفرد، وهو حاليًا مشفّر النص الوحيد الذي تستطيع منظومة MLX تحميله. وبإشغاله نحو 34 غيغابايت، فهو — لا المحوّل — ما يحدّد إمكانية تشغيل التطبيق على جهاز بعينه: تكلفة ثابتة لا يخفّضها أي تكميم، ولا يوفّر محوّل أصغر أكثر من 29 غيغابايت في مقابلها. غير أن الأوزان وحدها ليست المعيار — انظر تبويب المتطلبات لمعرفة ما يحتاجه الجهاز فعلًا. ويُثبَّت بجوار مشفّرات VAE داخل FL2VA/.',
    'ja': 'bfloat16 の Qwen3-VL-32B。H3 はその第 50 層の隠れ状態を読みます。単体では最大のダウンロードで、現在 MLX パイプラインが読み込める唯一のテキストエンコーダです。常駐約 34 GB で、このアプリがその Mac で動くかどうかを決めるのはトランスフォーマーではなくこちらです。量子化では減らせない固定費で、小さいトランスフォーマーに替えても最大 29 GB しか浮きません。ただし重みだけが判断材料ではありません。実際に必要な量は「動作要件」タブをご覧ください。VAE と同じ FL2VA/ に入ります。',
    'ko': 'bfloat16 Qwen3-VL-32B — H3는 이 모델의 50번째 층 은닉 상태를 읽습니다. 단일 항목으로 가장 큰 다운로드이며, 현재 MLX 파이프라인이 불러올 수 있는 유일한 텍스트 인코더입니다. 상주 약 34 GB로, 이 앱이 특정 Mac에서 돌아가는지를 정하는 것은 트랜스포머가 아니라 이것입니다. 양자화로도 줄지 않는 고정 비용이며, 더 작은 트랜스포머로 바꿔도 최대 29 GB밖에 아끼지 못합니다. 다만 가중치만으로는 판단할 수 없습니다. 실제로 필요한 양은 요구 사항 탭을 보십시오. VAE와 나란히 FL2VA/ 에 설치됩니다.',
    'th': 'Qwen3-VL-32B แบบ bfloat16 — H3 อ่านสถานะซ่อนของชั้นที่ 50 เป็นไฟล์ดาวน์โหลดเดี่ยวที่ใหญ่ที่สุด และตอนนี้เป็นตัวเข้ารหัสข้อความเพียงตัวเดียวที่ไปป์ไลน์ MLX โหลดได้ ด้วยหน่วยความจำราว 34 GB สิ่งที่ตัดสินว่าแอปนี้รันบน Mac เครื่องใดได้คือตัวนี้ ไม่ใช่ transformer เป็นต้นทุนคงที่ที่การทำ quantization ก็ลดไม่ได้ และเปลี่ยนไปใช้ transformer ที่เล็กลงก็ประหยัดได้มากสุด 29 GB แต่ลำพังไฟล์น้ำหนักไม่ใช่เกณฑ์ตัดสิน ดูว่าเครื่องต้องมีเท่าไรจริง ๆ ได้ที่แท็บความต้องการของระบบ และติดตั้งไว้ข้าง VAE ใน FL2VA/',
    'yue-Hant': 'bfloat16 版嘅 Qwen3-VL-32B——H3 讀佢第 50 層嘅隱藏狀態。單一檔案入面最大嘅下載，亦係而家 MLX pipeline 唯一載入到嘅文字編碼器。常駐大概 34 GB，所以決定呢個 App 喺部 Mac 跑唔跑到嘅係佢，唔係 transformer：呢個係量化都減唔到嘅固定成本，換細啲嘅 transformer 最多都係慳到 29 GB。不過單睇權重唔準——實際要幾多，睇「系統需求」嗰版。佢會同 VAE 一齊裝喺 FL2VA/ 入面。',
    'en-SG': 'Qwen3-VL-32B in bfloat16 — H3 reads its 50th-layer hidden states. Biggest single download, and right now the only text encoder the MLX pipeline can load. About 34 GB resident, so this one — not the transformer — decides whether this app can run on your Mac: a fixed cost no quantization brings down, against which a smaller transformer saves you 29 GB at most. But weights alone are not the test — see the Requirements tab for what a machine actually needs. It installs beside the VAEs inside FL2VA/.',
})
add("model.fl2va.q4", {
...
add("summary.eta.footnote.cached", {
    "en": "Step reuse is on, so expect less than this — how much less depends "
          "on the clip.",
    "zh-Hant": "已開啟步驟重用，實際用時會比這短；短多少視內容而定。",
    "zh-Hans": "已开启步骤复用，实际用时会比这短；短多少视内容而定。",
    "de": "Schrittwiederverwendung ist aktiv, es wird also weniger — wie viel "
          "weniger, hängt vom Clip ab.",
    "ar": "إعادة استخدام الخطوات مفعّلة، لذا توقّع أقل من ذلك؛ ومقدار الفرق "
          "يعتمد على المقطع.",
    "ja": "ステップの再利用が有効なので、実際はこれより短くなります。どれだけ短くなるかは"
          "クリップ次第です。",
    "ko": "스텝 재사용이 켜져 있어 실제로는 이보다 짧습니다. 얼마나 짧아질지는 "
          "클립에 따라 다릅니다.",
    "th": "เปิดการใช้สเต็ปซ้ำอยู่ จึงน่าจะใช้เวลาน้อยกว่านี้ มากน้อยขึ้นกับคลิป",
    "yue-Hant": "開咗步驟重用，所以實際會快過呢個；快幾多就睇內容。",
    "en-SG": "Step reuse is on, so expect less than this — how much less depends "
             "on the clip.",
...
add("format.res.preview.detail", {
    "en": "Rendered at this size, not shrunk afterwards — much faster, and far "
          "less memory. It shows whether a prompt works, not what the shot will "
          "be: a different canvas draws different noise, so the same seed gives "
          "a different take, and H3 is trained at 768.",
    "zh-Hant": "直接以此尺寸算繪，而非事後縮小——速度快得多，記憶體也省得多。"
               "它能看出提示詞是否奏效，卻看不出成片長什麼樣："
               "畫布尺寸不同，抽出的雜訊也不同，因此同一種子會得到不同的一次拍攝；"
               "何況 H3 是以 768 訓練的。",
    "zh-Hans": "直接以此尺寸渲染，而非事后缩小——速度快得多，内存也省得多。"
               "它能看出提示词是否奏效，却看不出成片长什么样："
               "画布尺寸不同，抽取的噪声也不同，因此同一种子会得到不同的一次拍摄；"
               "何况 H3 是以 768 训练的。",
    "de": "Wird in dieser Größe gerendert, nicht nachträglich verkleinert — "
          "deutlich schneller und mit viel weniger Speicher. Es zeigt, ob ein "
          "Prompt funktioniert, nicht wie die Einstellung aussehen wird: eine "
          "andere Leinwand zieht anderes Rauschen, dieselbe Seed ergibt also "
          "einen anderen Take — und H3 ist auf 768 trainiert.",
    "ar": "يُنتَج بهذا الحجم مباشرةً لا بالتصغير لاحقًا — أسرع بكثير وبذاكرة أقل "
          "بكثير. يُظهر ما إذا كان الوصف ناجحًا، لا كيف ستبدو اللقطة: فاختلاف "
          "المساحة يعني ضجيجًا مختلفًا، وبالتالي تعطي البذرة نفسها لقطة مختلفة، "
          "فضلًا عن أن H3 دُرِّب على 768.",
    "ja": "この大きさで直接生成します。後から縮小するのではありません。大幅に速く、"
          "メモリもはるかに少なくて済みます。プロンプトが効くかどうかは分かりますが、"
          "仕上がりの画は分かりません。キャンバスが変わればノイズも変わるため、"
          "同じシードでも別のテイクになります。H3 は 768 で学習している点も同じ向きに働きます。",
    "ko": "나중에 줄이는 것이 아니라 이 크기로 직접 생성합니다. 훨씬 빠르고 메모리도 "
          "훨씬 적게 씁니다. 프롬프트가 통하는지는 알 수 있지만 최종 장면이 어떨지는 "
          "알 수 없습니다. 캔버스가 다르면 노이즈도 달라져 같은 시드라도 다른 테이크가 "
          "되고, H3는 768로 학습되었습니다.",
    "th": "เรนเดอร์ที่ขนาดนี้โดยตรง ไม่ใช่ย่อทีหลัง จึงเร็วกว่ามากและใช้หน่วยความจำน้อยกว่ามาก "
          "บอกได้ว่าพรอมต์ใช้ได้ไหม แต่บอกไม่ได้ว่าภาพจริงจะเป็นอย่างไร เพราะผืนภาพต่างขนาด"
          "จะสุ่มนอยส์ต่างกัน ซีดเดียวกันจึงได้เทกคนละแบบ อีกทั้ง H3 ฝึกมาที่ 768",
    "yue-Hant": "直接用呢個尺寸算，唔係之後先縮細——快好多，記憶體都慳好多。"
                "佢睇得出提示詞得唔得，但睇唔出成片個樣："
                "畫布唔同，抽出嚟嘅雜訊都唔同，所以同一個種子會出到另一個 take；"
                "而且 H3 係用 768 訓練嘅。",
    "en-SG": "Rendered at this size, not shrunk afterwards — much faster, and far "
             "less memory. It shows whether a prompt works, not what the shot will "
             "be: a different canvas draws different noise, so the same seed gives "
             "a different take, and H3 is trained at 768.",
}, note="Shared by every tier below native. Must not imply a preview previews "
        "the final shot: the noise is drawn to the latent shape, so changing "
        "resolution changes the content outright, not just its size. An earlier "
        "version said to fix the seed to compare them, which invited exactly "
        "that misreading.")
add("sampling.cache", {
    "en": "Step reuse",
    "zh-Hant": "步驟重用",
    "zh-Hans": "步骤复用",
    "de": "Schrittwiederverwendung",
    "ar": "إعادة استخدام الخطوات",
    "ja": "ステップの再利用",
    "ko": "스텝 재사용",
    "th": "การใช้สเต็ปซ้ำ",
    "yue-Hant": "步驟重用",
    "en-SG": "Step reuse",
}, note="Labels the ComfyUI-only picker choosing EasyCache, LazyCache or off. "
        "Named for what it does, not for the nodes: the node names are the "
        "options themselves and stay in English.")
add("sampling.cache.off", {
    "en": "Off",
    "zh-Hant": "關閉",
    "zh-Hans": "关闭",
    "de": "Aus",
...
add("sampling.cache.off.detail", {
    "en": "Every step is computed. The reference path, and what the time "
          "estimate assumes.",
    "zh-Hant": "每一步都完整計算。這是基準路徑，時間估算也以此為準。",
    "zh-Hans": "每一步都完整计算。这是基准路径，时间估算也以此为准。",
    "de": "Jeder Schritt wird berechnet. Der Referenzpfad, von dem auch die "
          "Zeitschätzung ausgeht.",
    "ar": "تُحسب كل خطوة. هذا هو المسار المرجعي الذي يفترضه تقدير الوقت.",
    "ja": "すべてのステップを計算します。基準となる経路で、所要時間の見積もりもこれを前提にしています。",
    "ko": "모든 스텝을 계산합니다. 기준 경로이며 예상 시간도 이를 전제로 합니다.",
    "th": "คำนวณทุกสเต็ป เป็นเส้นทางอ้างอิงและเป็นสิ่งที่การประมาณเวลาใช้เป็นฐาน",
    "yue-Hant": "每一步都計算晒。呢個係基準路徑，時間估算都係咁計。",
    "en-SG": "Every step is computed. The reference path, and what the time "
             "estimate assumes.",
})
add("sampling.cache.on", {
    "en": "On",
    "zh-Hant": "開啟",
    "zh-Hans": "开启",
    "de": "Ein",
    ...
    "yue-Hant": "開咗",
    "en-SG": "On",
}, note="The MLX engine has one implementation rather than a choice of nodes, "
        "so its option is simply on.")
add("sampling.cache.reuse.detail", {
    "en": "Reuses a step when its output has barely moved. Faster, and the "
          "clip will differ from the same seed rendered in full — meant for "
          "previews. Halation's own, applied around the MLX port; the saving "
          "grows with the step count.",
    "zh-Hant": "當某一步的輸出幾乎沒有變化時便重用它。速度較快，"
               "但成品會與同一種子的完整算繪有所不同，適合用於預覽。"
               "這是 Halation 自行加在 MLX 移植版外層的做法；步數越多，省下的時間越多。",
    "zh-Hans": "当某一步的输出几乎没有变化时便复用它。速度较快，"
               "但成品会与同一种子的完整渲染有所不同，适合用于预览。"
               "这是 Halation 自行加在 MLX 移植版外层的做法；步数越多，省下的时间越多。",
    "de": "Verwendet einen Schritt wieder, wenn sich sein Ergebnis kaum "
          "verändert hat. Schneller, und der Clip weicht von derselben "
          "vollständig gerenderten Seed ab — für Vorschauen gedacht. Halations "
          "eigene Umsetzung um die MLX-Portierung herum; die Ersparnis wächst "
          "mit der Schrittzahl.",
    "ar": "يعيد استخدام الخطوة عندما لا يكاد ناتجها يتغير. أسرع، وسيختلف المقطع "
          "عن النتيجة الكاملة للبذرة نفسها — وهو مخصص للمعاينة. هذه إضافة من "
          "Halation حول منفذ MLX، ويزداد التوفير كلما زاد عدد الخطوات.",
    "ja": "出力がほとんど変化していないステップを再利用します。高速になりますが、"
          "同じシードを完全に生成した場合とは結果が変わります。プレビュー向けです。"
          "MLX 版を包む Halation 独自の実装で、ステップ数が多いほど節約も大きくなります。",
    "ko": "출력이 거의 변하지 않은 스텝을 재사용합니다. 더 빠르지만 같은 시드를 "
          "전부 계산한 결과와는 달라집니다. 미리보기용입니다. MLX 포팅을 감싸는 "
          "Halation 자체 구현이며, 스텝 수가 많을수록 절약도 커집니다.",
    "th": "ใช้สเต็ปเดิมซ้ำเมื่อผลลัพธ์แทบไม่เปลี่ยน เร็วขึ้น และคลิปจะต่างจากการเรนเดอร์เต็ม"
          "ด้วยซีดเดียวกัน จึงเหมาะกับการดูตัวอย่าง เป็นส่วนที่ Halation เพิ่มหุ้มรอบพอร์ต MLX "
          "ยิ่งสเต็ปมากยิ่งประหยัดมาก",
    "yue-Hant": "如果某一步嘅輸出幾乎冇變，就重用返佢。快啲，但出嚟嘅片會同同一個種子"
                "完整算出嚟嘅唔同，係用嚟預覽嘅。呢個係 Halation 自己包喺 MLX 移植版"
                "外面嘅做法；步數越多，慳得越多。",
    "en-SG": "Reuses a step when its output has barely moved. Faster, and the "
             "clip will differ from the same seed rendered in full — meant for "
             "previews. Halation's own, applied around the MLX port; the saving "
             "grows with the step count.",
}, note="The MLX engine's step reuse. Same algorithm as EasyCache, but "
        "Halation's code rather than ComfyUI's, so it does not claim the name.")
add("sampling.cache.easy.detail", {
    "en": "Reuses a step when its output has barely moved. Faster, and the "
          "clip will differ from the same seed rendered in full — meant for "
          "previews. The saving grows with the step count.",
    "zh-Hant": "當某一步的輸出幾乎沒有變化時便重用它。速度較快，"
               "但成品會與同一種子的完整算繪有所不同，適合用於預覽。步數越多，省下的時間越多。",
    "zh-Hans": "当某一步的输出几乎没有变化时便复用它。速度较快，"
               "但成品会与同一种子的完整渲染有所不同，适合用于预览。步数越多，省下的时间越多。",
    "de": "Verwendet einen Schritt wieder, wenn sich sein Ergebnis kaum "
          "verändert hat. Schneller, und der Clip weicht von derselben "
          "vollständig gerenderten Seed ab — für Vorschauen gedacht. Die "
          "Ersparnis wächst mit der Schrittzahl.",
    "ar": "يعيد استخدام الخطوة عندما لا يكاد ناتجها يتغير. أسرع، وسيختلف المقطع "
          "عن النتيجة الكاملة للبذرة نفسها — وهو مخصص للمعاينة. ويزداد التوفير "
          "كلما زاد عدد الخطوات.",
    "ja": "出力がほとんど変化していないステップを再利用します。高速になりますが、"
          "同じシードを完全に生成した場合とは結果が変わります。プレビュー向けです。"
          "ステップ数が多いほど節約も大きくなります。",
    "ko": "출력이 거의 변하지 않은 스텝을 재사용합니다. 더 빠르지만 같은 시드를 "
          "전부 계산한 결과와는 달라집니다. 미리보기용입니다. 스텝 수가 많을수록 "
          "절약도 커집니다.",
    "th": "ใช้สเต็ปเดิมซ้ำเมื่อผลลัพธ์แทบไม่เปลี่ยน เร็วขึ้น และคลิปจะต่างจากการเรนเดอร์เต็ม"
          "ด้วยซีดเดียวกัน จึงเหมาะกับการดูตัวอย่าง ยิ่งสเต็ปมากยิ่งประหยัดมาก",
    "yue-Hant": "如果某一步嘅輸出幾乎冇變，就重用返佢。快啲，但出嚟嘅片會同同一個種子"
                "完整算出嚟嘅唔同，係用嚟預覽嘅。步數越多，慳得越多。",
    "en-SG": "Reuses a step when its output has barely moved. Faster, and the "
             "clip will differ from the same seed rendered in full — meant for "
             "previews. The saving grows with the step count.",
}, note="EasyCache. Keep \"preview\" prominent: this trades quality for time.")
add("sampling.cache.lazy.detail", {
    "en": "A simpler reuse rule than EasyCache. Usually worse for the same "
          "time saved, occasionally better. Try it if EasyCache spoils a shot.",
    "zh-Hant": "比 EasyCache 更簡單的重用規則。在省下同樣時間的前提下通常較差，"
               "偶爾較好。若 EasyCache 毀了某個鏡頭，可以試試這個。",
    "zh-Hans": "比 EasyCache 更简单的复用规则。在省下同样时间的前提下通常较差，"
               "偶尔较好。若 EasyCache 毁了某个镜头，可以试试这个。",
    "de": "Eine einfachere Wiederverwendungsregel als EasyCache. Bei gleicher "
          "Zeitersparnis meist schlechter, gelegentlich besser. Einen Versuch "
          "wert, wenn EasyCache eine Einstellung verdirbt.",
    "ar": "قاعدة إعادة استخدام أبسط من EasyCache. أسوأ عادةً مقابل التوفير نفسه، "
          "وأفضل أحيانًا. جرّبها إذا أفسد EasyCache لقطة ما.",
    "ja": "EasyCache より単純な再利用の判定です。同じ時間短縮なら通常は劣りますが、"
          "まれに上回ります。EasyCache で画が崩れるときに試してください。",
    "ko": "EasyCache보다 단순한 재사용 규칙입니다. 같은 시간을 아낄 때 대체로 더 "
          "나쁘지만 가끔 더 낫습니다. EasyCache가 장면을 망칠 때 시도해 보세요.",
    "th": "กฎการใช้ซ้ำที่ง่ายกว่า EasyCache โดยทั่วไปให้ผลแย่กว่าที่เวลาประหยัดเท่ากัน "
          "แต่บางครั้งก็ดีกว่า ลองใช้เมื่อ EasyCache ทำให้ภาพเสีย",
    "yue-Hant": "比 EasyCache 簡單啲嘅重用規則。慳返同樣時間嘅話通常差啲，偶爾好啲。"
                "如果 EasyCache 搞衰咗個鏡頭，可以試下呢個。",
    "en-SG": "A simpler reuse rule than EasyCache. Usually worse for the same "
             "time saved, occasionally better. Try it if EasyCache spoils a shot.",
}, note="LazyCache. ComfyUI's own description calls it a homebrew EasyCache "
        "that works worse overall but has universal compatibility.")
...
add("memory.table.note", {
    'en': 'Weights are only part of it: generating needs working memory on top, and how much depends on the engine — a measured MLX render peaked at 2.4 times its weight figure, a ComfyUI one at 1.41, because ComfyUI loads its models in stages and never holds them all at once. The verdicts allow for that. “Tight” means the peak clears the usable share but still fits in installed memory, so macOS pages some of it out and the render finishes anyway — measured, a 120 GB peak on a 128 GB Mac used 16 GB of swap and completed. “Swaps” means it exceeds installed memory, and a render that would take an hour takes far longer. The usable share is what Metal reports for the machine — roughly three quarters to five sixths of what is installed, higher on the larger configurations.',
    'zh-Hant': '權重只是其中一部分：生成時還需要額外的工作記憶體，需要多少則視引擎而定——實測 MLX 算圖的尖峰是權重數字的 2.4 倍，ComfyUI 只有 1.41 倍，因為 ComfyUI 分階段載入模型，不會同時全部留著。上表的判定已計入這點。「吃緊」是指尖峰超出可用額度、但仍裝得進已安裝的記憶體，macOS 會把一部分換出，算圖照樣完成——實測在 128 GB 機器上尖峰 120 GB，用掉 16 GB 置換空間並順利跑完。「置換」是指超出已安裝的記憶體，本來一小時的算圖會拖得久很多。可用比例取自 Metal 對該機器的回報——大約是安裝容量的四分之三到六分之五，容量越大比例越高。',
    'zh-Hans': '权重只是其中一部分：生成时还需要额外的工作内存，需要多少则视引擎而定——实测 MLX 渲染的峰值是权重数字的 2.4 倍，ComfyUI 只有 1.41 倍，因为 ComfyUI 分阶段载入模型，不会同时全部留着。上表的判定已计入这点。“吃紧”是指峰值超出可用额度、但仍装得进已安装的内存，macOS 会把一部分换出，渲染照样完成——实测在 128 GB 机器上峰值 120 GB，用掉 16 GB 交换空间并顺利跑完。“交换”是指超出已安装的内存，本来一小时的渲染会拖得久很多。可用比例取自 Metal 对该机器的报告——大约是安装容量的四分之三到六分之五，容量越大比例越高。',
    'de': 'Die Gewichte sind nur ein Teil: Das Erzeugen braucht zusätzlich Arbeitsspeicher, und wie viel, hängt von der Engine ab — ein gemessener MLX-Render erreichte in der Spitze das 2,4-Fache seines Gewichtswerts, ein ComfyUI-Render nur das 1,41-Fache, weil ComfyUI seine Modelle gestaffelt lädt und nie alle gleichzeitig hält. Die Urteile berücksichtigen das. „Knapp“ heißt, die Spitze übersteigt den nutzbaren Anteil, passt aber noch in den verbauten Speicher: macOS lagert einen Teil aus und der Render läuft dennoch durch — gemessen brauchte eine Spitze von 120 GB auf einem Mac mit 128 GB 16 GB Swap und wurde fertig. „Swap“ heißt, es übersteigt den verbauten Speicher, und ein Render von einer Stunde dauert dann weit länger. Der nutzbare Anteil ist der, den Metal für die Maschine meldet — etwa drei Viertel bis fünf Sechstel des Verbauten, bei größeren Konfigurationen mehr.',
    ...
    'en-SG': 'Weights are only part of the story: generating needs working memory on top, and how much depends on the engine — one measured MLX render peaked at 2.4 times its weight figure, a ComfyUI one at 1.41, because ComfyUI loads its models in stages and never holds everything at once. The verdicts already count that in. “Tight” means the peak goes past the usable share but still fits inside installed memory, so macOS pages some of it out and the render still finishes — measured, a 120 GB peak on a 128 GB Mac used 16 GB of swap and completed. “Swaps” means it goes past installed memory altogether, and a render that would take an hour takes very much longer. The usable share is whatever Metal reports for the machine — roughly three quarters to five sixths of what is installed, higher on the bigger configurations.',
        ...
        "explained. Each multiplier comes from one measured render of that "
        "engine (MLX: 50 GB of weights peaking at 120 GB on a 128 GB Mac; "
        "ComfyUI: 55 GB peaking at 77.6 GB), so they are rules of thumb, not "
        "specifications. Keep the quoted words identical to "
        "memory.verdict.tight and memory.verdict.swaps.")
...
add("settings.disk", {
    "en": "Min disk space required",
    "zh-Hant": "所需最小磁碟空間", "zh-Hans": "所需最小磁盘空间",
    "de": "Mindestens nötiger Speicherplatz", "ar": "أدنى مساحة قرص مطلوبة",
    "ja": "必要な最小ディスク容量", "ko": "필요한 최소 디스크 공간",
    "th": "พื้นที่ดิสก์ขั้นต่ำที่ต้องมี", "yue-Hant": "要用嘅最少磁碟空間",
    "en-SG": "Min disk space required",
}, note="Heads the disk table, beside the memory one, in Settings and in the "
        "welcome dialog. It is a floor, not an estimate: the smallest set of "
        "weights that can render anything, plus the swap that machine will need.")
...
add("disk.table.note", {
    "en": "The weights are the whole of it: %@ for the smallest usable set — the "
          ...
          "680 MB of Python runtime. A render's own temporary files are a few "
          "megabytes, because frames are piped straight to the encoder and never "
          "written out. Setup transiently needs 3.56 GB more for uv's download "
    ...
    "zh-Hant": "空間幾乎全用在權重上：最小可用組合約 %@——共用 VAE、文字編碼器與 4-bit transformer——再加上約 680 MB 的 Python 執行環境。算圖本身的暫存檔只有幾 MB，因為影格是直接餵給編碼器，不會寫到磁碟。初次設定期間還會多用 3.56 GB 作為 uv 的下載快取，之後可在「快取」分頁清除。參考素材模式需要另一整套權重，約 60 GB，未計入此表。",
    "zh-Hans": "空间几乎全用在权重上：最小可用组合约 %@——共用 VAE、文本编码器与 4-bit transformer——再加上约 680 MB 的 Python 运行环境。渲染本身的暂存文件只有几 MB，因为帧是直接喂给编码器，不会写到磁盘。初次设置期间还会多用 3.56 GB 作为 uv 的下载缓存，之后可在“缓存”分页清除。参考素材模式需要另一整套权重，约 60 GB，未计入此表。",
    "de": "Die Gewichte sind fast alles: %@ für den kleinsten brauchbaren Satz — die "
          ...
          "680 MB Python-Laufzeit. Die Zwischendateien eines Renders sind wenige "
    ...
    "ar": "الأوزان هي كل شيء تقريبًا: %@ لأصغر مجموعة صالحة — وحدات VAE المشتركة "
          "ومرمِّز النص والمحوّل بأربع بتات — إضافة إلى نحو 680 ميغابايت لبيئة Python. "
          "أما ملفات التصيير المؤقتة فبضعة ميغابايتات فقط، لأن الإطارات تُمرَّر إلى "
          "المرمِّز مباشرة ولا تُكتب أبدًا. ويحتاج الإعداد مؤقتًا إلى 3.56 غيغابايت "
    ...
    "ja": "容量のほとんどは重みです。最小構成で約 %@——共有 VAE、テキストエンコーダ、4-bit transformer——に Python 実行環境の約 680 MB を加えた程度です。レンダリング自体の一時ファイルは数 MB しかありません。フレームはそのままエンコーダへ渡され、ディスクには書かれないからです。初回セットアップ中は uv のダウンロードキャッシュに一時的にさらに 3.56 GB を使い、あとから「キャッシュ」タブで空にできます。参考素材モードには別の重み一式、約 60 GB が必要で、この表には含めていません。",
    "ko": "용량은 거의 전부 가중치입니다. 가장 작은 실용 구성이 약 %@ — 공용 VAE, 텍스트 인코더, 4비트 트랜스포머 — 에 Python 런타임 약 680 MB를 더한 정도입니다. 렌더링 자체의 임시 파일은 수 MB뿐인데, 프레임이 인코더로 바로 전달되고 디스크에 쓰이지 않기 때문입니다. 설치 중에는 uv 다운로드 캐시로 3.56 GB를 일시적으로 더 쓰며, 나중에 캐시 탭에서 비울 수 있습니다. 참조 모드에는 약 60 GB의 두 번째 가중치 모음이 필요하고, 이 표에는 넣지 않았습니다.",
    "th": "พื้นที่แทบทั้งหมดคือไฟล์น้ำหนัก: ราว %@ สำหรับชุดที่เล็กที่สุดที่ใช้งานได้ — VAE ที่ใช้ร่วมกัน "
          "ตัวเข้ารหัสข้อความ และ transformer แบบ 4 บิต — บวกรันไทม์ Python ราว 680 MB "
          "ไฟล์ชั่วคราวของการเรนเดอร์มีเพียงไม่กี่เมกะไบต์ เพราะเฟรมถูกส่งตรงไปยังตัวเข้ารหัสและไม่เคยเขียนลงดิสก์ "
          "ระหว่างการตั้งค่าจะใช้เพิ่มชั่วคราวอีก 3.56 GB สำหรับแคชดาวน์โหลดของ uv ซึ่งล้างได้ภายหลังจากแท็บแคช "
          "โหมดไฟล์อ้างอิงต้องใช้ไฟล์น้ำหนักอีกชุดราว 60 GB ซึ่งไม่ได้รวมไว้ที่นี่",
    "yue-Hant": "空間差不多全部用喺權重：最細嘅可用組合大概 %@——共用 VAE、文字編碼器同 4-bit transformer——再加大概 680 MB 嘅 Python 執行環境。算圖本身嘅暫存檔得幾 MB，因為影格係直接餵去編碼器，唔會寫落碟。初次設定期間仲會多用 3.56 GB 做 uv 嘅下載快取，之後喺「快取」嗰版清得。參考素材模式要另一整套權重，大概 60 GB，呢個表冇計。",
    "en-SG": "The weights are almost all of it: about %@ for the smallest usable set — "
             ...
             "680 MB of Python runtime. A render's own temp files are only a few "
             "megabytes, because frames go straight into the encoder and never get "
             "written out. Setup needs 3.56 GB more for a while for uv's download "
...
# ── Disk breakdown ───────────────────────────────────────────────────────────
add("disk.table.breakdown", {
    "en": "What makes up the smallest usable set",
    "zh-Hant": "最小可用組合的組成", "zh-Hans": "最小可用组合的构成",
    "de": "Woraus der kleinste brauchbare Satz besteht",
    "ar": "مِمَّ تتكوّن أصغر مجموعة صالحة للاستخدام",
    "ja": "使える最小構成の内訳", "ko": "쓸 수 있는 최소 구성의 내역",
    "th": "ชุดเล็กที่สุดที่ใช้งานได้ประกอบด้วยอะไรบ้าง",
    "yue-Hant": "最細嘅可用組合有咩", "en-SG": "What makes up the smallest usable set",
}, note="Heads the itemised list above the per-machine disk table. 'Usable' "
        "means it can render at all, not that it renders well.")
add("disk.item.vae", {
    "en": "Shared VAE bundle", "zh-Hant": "共用 VAE 套件", "zh-Hans": "共用 VAE 套件",
    "de": "Gemeinsames VAE-Paket", "ar": "حزمة VAE المشتركة",
    "ja": "共有 VAE 一式", "ko": "공용 VAE 묶음", "th": "ชุด VAE ที่ใช้ร่วมกัน",
    "yue-Hant": "共用嘅 VAE 套件", "en-SG": "Shared VAE bundle",
}, note="The video and audio VAEs, plus the tokenizer and processor. Shared "
        "because every task needs them whichever transformer is chosen.")
add("disk.item.encoder", {
    "en": "Text encoder (bfloat16)", "zh-Hant": "文字編碼器（bfloat16）",
    "zh-Hans": "文本编码器（bfloat16）", "de": "Text-Encoder (bfloat16)",
    "ar": "مشفّر النص (bfloat16)", "ja": "テキストエンコーダ（bfloat16）",
    "ko": "텍스트 인코더(bfloat16)", "th": "ตัวเข้ารหัสข้อความ (bfloat16)",
    "yue-Hant": "文字編碼器（bfloat16）", "en-SG": "Text encoder (bfloat16)",
}, note="The largest line by far, and the reason the floor is where it is. "
        "Keep the format in the label: it is the only build MLX can load.")
add("disk.item.transformer", {
    "en": "FL2VA transformer, 4-bit", "zh-Hant": "FL2VA transformer，4-bit",
    "zh-Hans": "FL2VA transformer，4-bit", "de": "FL2VA-Transformer, 4 Bit",
    "ar": "محوّل FL2VA بدقة 4-bit", "ja": "FL2VA トランスフォーマー（4 ビット）",
    "ko": "FL2VA 트랜스포머, 4비트", "th": "FL2VA transformer แบบ 4 บิต",
    "yue-Hant": "FL2VA transformer，4-bit", "en-SG": "FL2VA transformer, 4-bit",
}, note="The smallest transformer, because this table is a floor. A larger "
        "quantization adds to this line and nothing else.")
add("disk.item.runtime", {
    "en": "Python runtime, uv and the app",
    "zh-Hant": "Python 執行環境、uv 與 App", "zh-Hans": "Python 运行环境、uv 与 App",
    "de": "Python-Laufzeit, uv und die App", "ar": "بيئة تشغيل Python وuv والتطبيق",
    "ja": "Python ランタイム、uv、アプリ本体", "ko": "Python 런타임, uv, 앱 자체",
    "th": "รันไทม์ Python, uv และตัวแอป", "yue-Hant": "Python 執行環境、uv 同個 App",
    "en-SG": "Python runtime, uv and the app",
}, note="Everything that is not a model weight. Measured, and small enough "
        "beside the weights that the exact figure hardly matters.")
add("disk.item.persistent", {
    "en": "Kept on disk", "zh-Hant": "長期佔用", "zh-Hans": "长期占用",
    "de": "Dauerhaft belegt", "ar": "يبقى على القرص", "ja": "ディスクに残る分",
    "ko": "디스크에 남는 양", "th": "ที่อยู่บนดิสก์ถาวร", "yue-Hant": "會一直佔住",
    "en-SG": "Kept on disk",
}, note="Subtotal: the lines above it, which stay for as long as the app is "
        "installed. Contrast with the transient lines below.")
add("disk.item.uvCache", {
    "en": "uv download cache (setup only)",
    "zh-Hant": "uv 下載快取（僅安裝時）", "zh-Hans": "uv 下载缓存（仅安装时）",
    "de": "uv-Download-Cache (nur bei der Einrichtung)",
    "ar": "ذاكرة تنزيل uv المؤقتة (أثناء الإعداد فقط)",
    "ja": "uv のダウンロードキャッシュ（セットアップ時のみ）",
    "ko": "uv 다운로드 캐시(설치할 때만)", "th": "แคชดาวน์โหลดของ uv (เฉพาะตอนติดตั้ง)",
    "yue-Hant": "uv 下載快取（淨係裝嗰陣）", "en-SG": "uv download cache (setup only)",
}, note="Transient: needed while the runtime is built, and the Cache tab can "
        "empty it afterwards. Say 'setup only' so nobody budgets for it twice.")
add("disk.item.scratch", {
...
}, note="Surprisingly tiny, and worth showing for exactly that reason: frames "
        "are piped straight to the encoder, so they are never written out.")
add("disk.item.setupPeak", {
    "en": "Peak, during setup", "zh-Hant": "尖峰（安裝過程中）", "zh-Hans": "峰值（安装过程中）",
    "de": "Spitze während der Einrichtung", "ar": "الذروة أثناء الإعداد",
    "ja": "ピーク（セットアップ中）", "ko": "최대치(설치 중)",
    "th": "จุดสูงสุด ระหว่างติดตั้ง", "yue-Hant": "尖峰（裝緊嗰陣）",
...
add("settings.cache.interactions", {
    "en": "Interaction log", "zh-Hant": "操作記錄", "zh-Hans": "操作记录",
    "de": "Interaktionsprotokoll", "ar": "سجل التفاعل", "ja": "操作ログ",
    "ko": "조작 기록", "th": "บันทึกการใช้งาน", "yue-Hant": "操作記錄",
    "en-SG": "Interaction log",
}, note="A Cache tab category: the record of what was clicked, kept for "
        "debugging. Deleting it loses history and nothing else.")
add("settings.cache.interactions.detail", {
    "en": "Every button, menu and slider this app has recorded, for working out "
          "afterwards what changed and when. Nothing you typed is in it — a "
          "prompt is recorded as its length, never its text.",
    "zh-Hant": "本 App 記下的每次按鈕、選單與滑桿操作，用來事後釐清什麼在何時改變。裡面沒有你輸入的內容——提示詞只記長度，不記文字。",
    "zh-Hans": "本 App 记下的每次按钮、菜单与滑块操作，用来事后理清什么在何时改变。里面没有你输入的内容——提示词只记长度，不记文字。",
    "de": "Jeder Knopf, jedes Menü und jeder Regler, den diese App aufgezeichnet "
          "hat, um hinterher nachvollziehen zu können, was sich wann geändert "
          "hat. Nichts Getipptes steht darin — ein Prompt wird als Länge "
          "festgehalten, nie als Text.",
    "ar": "كل زر وقائمة وشريط تمرير سجّله هذا التطبيق، لمعرفة ما تغيّر ومتى. لا "
          "شيء مما كتبته موجود فيه — يُسجَّل الموجّه بطوله لا بنصّه.",
    "ja": "この App が記録したボタン・メニュー・スライダーの操作すべて。あとから何がいつ変わったのかを追うためのものです。入力した内容は入っていません。プロンプトは文字数だけを記録し、本文は残しません。",
    "ko": "이 App이 기록한 버튼·메뉴·슬라이더 조작 전부. 무엇이 언제 바뀌었는지 나중에 확인하기 위한 것입니다. 입력한 내용은 들어 있지 않습니다. 프롬프트는 글자 수만 기록하고 본문은 남기지 않습니다.",
    "th": "ทุกปุ่ม เมนู และแถบเลื่อนที่แอปนี้บันทึกไว้ เพื่อให้ย้อนดูได้ว่าอะไรเปลี่ยนเมื่อไร "
          "สิ่งที่คุณพิมพ์ไม่ได้อยู่ในนั้น — พรอมต์ถูกบันทึกเป็นความยาว ไม่ใช่ข้อความ",
    "yue-Hant": "呢個 App 記低嘅每次撳掣、揀餐單同拉滑桿，用嚟事後查返係咩喺幾時改咗。你打過嘅嘢唔喺入面——提示詞淨係記長度，唔記內容。",
    "en-SG": "Every button, menu and slider this app has recorded, for working "
             "out afterwards what changed and when. Nothing you typed is in it "
...
add("scratch.bar.title", {
    "en": "Swap + render scratch",
    "zh-Hant": "置換空間 + 算圖暫存", "zh-Hans": "交换空间 + 渲染暂存",
    "de": "Swap + Arbeitsdateien", "ar": "التبديل + ملفات التصيير المؤقتة",
    "ja": "スワップ + レンダリングの作業ファイル",
    "ko": "스왑 + 렌더링 작업 파일",
    "th": "พื้นที่สลับ + ไฟล์ชั่วคราวของการเรนเดอร์",
    "yue-Hant": "置換空間 + 算圖暫存檔",
    "en-SG": "Swap + render scratch",
}, note="Labels the disk bar under the memory chart on the Queue page. Both "
        "halves, because both grow during a render and only one of them is "
        "ever large: swap reached 66 GB here while the scratch stayed in "
        "megabytes.")
add("scratch.bar.summary", {
    "en": "Swap %1$@ · scratch %2$@ · %3$@ toward full · %4$@ free",
    "zh-Hant": "置換空間 %1$@ · 暫存 %2$@ · 距離寫滿 %3$@ · 剩餘 %4$@",
    "zh-Hans": "交换空间 %1$@ · 暂存 %2$@ · 距离写满 %3$@ · 剩余 %4$@",
    "de": "Swap %1$@ · Arbeitsdateien %2$@ · %3$@ bis voll · %4$@ frei",
    "ar": "التبديل %1$@ · الملفات المؤقتة %2$@ · %3$@ حتى الامتلاء · %4$@ متاحة",
    "ja": "スワップ %1$@ · 作業ファイル %2$@ · 満杯まで %3$@ · 空き %4$@",
    "ko": "스왑 %1$@ · 작업 파일 %2$@ · 가득 찰 때까지 %3$@ · %4$@ 남음",
    "th": "พื้นที่สลับ %1$@ · ไฟล์ชั่วคราว %2$@ · ถึงเต็ม %3$@ · เหลือ %4$@",
    "yue-Hant": "置換空間 %1$@ · 暫存 %2$@ · 距離寫滿 %3$@ · 仲有 %4$@",
    "en-SG": "Swap %1$@ · scratch %2$@ · %3$@ toward full · %4$@ free",
}, note={"content": "Four figures on the disk bar: swap in use, what the "
                    "render scratch holds, how far the two together have gone "
                    "toward filling the disk, and what is left. The percentage "
                    "is (swap + scratch) over (swap + scratch + free), so it "
                    "reads 100% only when the disk is full \u2014 it is not a "
...
# ── Halation's own terms ─────────────────────────────────────────────────────
add("licence.halation.title", {
    "en": "Terms of use for Halation itself",
    "zh-Hant": "Halation 本身的使用條款", "zh-Hans": "Halation 本身的使用条款",
    "de": "Nutzungsbedingungen für Halation selbst",
    "ar": "شروط استخدام Halation نفسه",
    "ja": "Halation 自体の利用条件", "ko": "Halation 자체의 이용 약관",
    "th": "ข้อกำหนดการใช้งานของ Halation เอง",
    "yue-Hant": "Halation 本身嘅使用條款", "en-SG": "Terms of use for Halation itself",
}, note="Heads the app's own terms, kept separate from the model licence above "
        "it: one is MiniMax's, this one is ours, and merging them would make "
        "neither clear.")
add("licence.halation.affiliation", {
    "en": "Halation is an independent application. It is not affiliated with, "
    ...
    "zh-Hant": "Halation 為獨立應用程式，與 MiniMax、ComfyUI 專案、Apple 或任何其軟體或模型可由本程式執行之第三方均無隸屬、背書或贊助關係。所有商標均屬其各自所有人所有。",
    "zh-Hans": "Halation 为独立应用程序，与 MiniMax、ComfyUI 项目、Apple 或任何其软件或模型可由本程序运行之第三方均无隶属、背书或赞助关系。所有商标均归其各自所有人所有。",
    "de": "Halation ist eine unabhängige Anwendung. Sie steht in keiner "
          "Verbindung zu MiniMax, dem ComfyUI-Projekt, Apple oder anderen "
          "Parteien, deren Software oder Modelle mit ihr ausgeführt werden "
          "können, und wird von diesen weder unterstützt noch gesponsert. Alle "
          "Marken sind Eigentum ihrer jeweiligen Inhaber.",
    "ar": "‏Halation تطبيق مستقل. وهو غير تابع لـ MiniMax أو مشروع ComfyUI أو "
          "Apple أو أي طرف آخر يمكن تشغيل برمجياته أو نماذجه به، ولا معتمد منهم "
          "ولا برعايتهم. وجميع العلامات التجارية ملك لأصحابها.",
    "ja": "Halation は独立したアプリケーションです。MiniMax、ComfyUI プロジェクト、Apple、その他本アプリで実行しうるソフトウェアまたはモデルの提供者のいずれとも、提携・推奨・後援の関係にありません。各商標はそれぞれの権利者に帰属します。",
    "ko": "Halation은 독립적인 응용 프로그램입니다. MiniMax, ComfyUI 프로젝트, Apple, 그 밖에 본 앱으로 실행할 수 있는 소프트웨어나 모델의 제공자와 제휴·보증·후원 관계가 없습니다. 모든 상표는 각 소유자의 재산입니다.",
    "th": "Halation เป็นแอปพลิเคชันอิสระ ไม่มีความเกี่ยวข้อง ไม่ได้รับการรับรอง "
          "และไม่ได้รับการสนับสนุนจาก MiniMax โครงการ ComfyUI Apple "
          "หรือบุคคลอื่นใดที่ซอฟต์แวร์หรือโมเดลของตนอาจถูกเรียกใช้ผ่านแอปนี้ "
          "เครื่องหมายการค้าทั้งหมดเป็นของเจ้าของแต่ละราย",
    "yue-Hant": "Halation 係獨立嘅應用程式，同 MiniMax、ComfyUI 專案、Apple，或者任何軟件或模型可以喺本程式行嘅一方，都冇隸屬、背書或者贊助關係。所有商標均屬各自擁有人所有。",
    "en-SG": "Halation is an independent application. It is not affiliated "
             "with, endorsed by, or sponsored by MiniMax, the ComfyUI project, "
             "Apple, or any other party whose software or models it can be used "
             "to run. All trademarks are the property of their respective owners.",
})
add("licence.halation.responsibility", {
    "en": "You are solely responsible for the media you generate and for how you "
          "use it, including compliance with the licences of any model you run, "
          "with applicable law, and with the rights of any person depicted or "
          "any work referenced.",
    "zh-Hant": "您對所生成之媒體及其使用方式負全部責任，包括遵守所執行模型之授權條款、適用法律，以及任何被描繪之人或被引用作品之權利。",
    "zh-Hans": "您对所生成之媒体及其使用方式负全部责任，包括遵守所运行模型之授权条款、适用法律，以及任何被描绘之人或被引用作品之权利。",
    "de": "Sie sind allein verantwortlich für die erzeugten Medien und deren "
          "Verwendung, einschließlich der Einhaltung der Lizenzen der "
          "ausgeführten Modelle, des geltenden Rechts sowie der Rechte "
          "abgebildeter Personen und referenzierter Werke.",
    "ar": "أنت وحدك المسؤول عن الوسائط التي تُنشئها وعن طريقة استخدامها، بما في "
          "ذلك الالتزام بتراخيص أي نموذج تشغّله، وبالقوانين المعمول بها، وبحقوق "
          "أي شخص يظهر فيها أو أي عمل يُستشهد به.",
    "ja": "生成した媒体およびその利用について、実行したモデルのライセンス、適用される法令、描写された人物や参照した著作物の権利の遵守を含め、すべての責任は利用者が負います。",
    "ko": "생성한 미디어와 그 이용에 대한 책임은 전적으로 이용자에게 있으며, 여기에는 실행한 모델의 라이선스, 관련 법령, 묘사된 인물이나 참조된 저작물의 권리 준수가 포함됩니다.",
    "th": "คุณรับผิดชอบแต่เพียงผู้เดียวต่อสื่อที่สร้างขึ้นและวิธีการนำไปใช้ "
          "รวมถึงการปฏิบัติตามสัญญาอนุญาตของโมเดลที่เรียกใช้ กฎหมายที่เกี่ยวข้อง "
          "และสิทธิของบุคคลที่ปรากฏหรือผลงานที่ถูกอ้างอิง",
    "yue-Hant": "你對生成嘅媒體同點樣用佢負全部責任，包括遵守所行模型嘅授權、適用法律，以及任何被描繪嘅人或被引用作品嘅權利。",
    "en-SG": "You are solely responsible for the media you generate and for how "
             "you use it, including compliance with the licences of any model "
             "you run, with applicable law, and with the rights of any person "
             "depicted or any work referenced.",
})
add("licence.halation.disclosure", {
    "en": "Where you publish, distribute or otherwise make generated media "
          "available, it is your responsibility to disclose that it was "
          "generated by artificial intelligence, by whatever notice, label or "
          "embedded marking the law or platform of distribution requires.",
    "zh-Hant": "當您發布、散布或以其他方式公開所生成之媒體時，應由您負責揭露其為人工智慧生成，並依法律或發布平台之要求，採用相應之聲明、標示或嵌入式標記。",
    "zh-Hans": "当您发布、分发或以其他方式公开所生成之媒体时，应由您负责披露其为人工智能生成，并依法律或发布平台之要求，采用相应之声明、标识或嵌入式标记。",
    "de": "Wenn Sie erzeugte Medien veröffentlichen, verbreiten oder anderweitig "
          "zugänglich machen, obliegt es Ihnen, deren Erzeugung durch "
          "künstliche Intelligenz offenzulegen — durch den Hinweis, die "
          "Kennzeichnung oder die eingebettete Markierung, die Gesetz oder "
          "Verbreitungsplattform verlangen.",
    "ar": "عند نشر الوسائط المُنشأة أو توزيعها أو إتاحتها بأي شكل، تقع عليك "
          "مسؤولية الإفصاح عن أنها من إنشاء الذكاء الاصطناعي، بالإشعار أو الوسم "
          "أو العلامة المضمّنة التي يقتضيها القانون أو منصة النشر.",
    "ja": "生成した媒体を公開・頒布その他の方法で提供する場合、それが人工知能により生成されたことを、法令または頒布先プラットフォームが求める表示・ラベル・埋め込み標識によって開示する責任は利用者にあります。",
    "ko": "생성한 미디어를 공개·배포하거나 그 밖의 방법으로 제공하는 경우, 그것이 인공지능으로 생성되었음을 법령 또는 배포 플랫폼이 요구하는 고지·표시·삽입 표식으로 밝힐 책임은 이용자에게 있습니다.",
    "th": "เมื่อคุณเผยแพร่ แจกจ่าย หรือทำให้สื่อที่สร้างขึ้นเข้าถึงได้โดยประการอื่น "
          "เป็นความรับผิดชอบของคุณที่จะเปิดเผยว่าสื่อนั้นสร้างโดยปัญญาประดิษฐ์ "
          "ด้วยข้อความแจ้ง ป้ายกำกับ หรือเครื่องหมายฝังตัวตามที่กฎหมายหรือแพลตฟอร์มกำหนด",
    "yue-Hant": "當你發布、散布或者用其他方式公開生成嘅媒體，你有責任披露佢係人工智能生成，並按法律或者發布平台嘅要求加上相應聲明、標示或者嵌入式標記。",
    "en-SG": "Where you publish, distribute or otherwise make generated media "
             "available, it is your responsibility to disclose that it was "
             "generated by artificial intelligence, by whatever notice, label "
             "or embedded marking the law or platform of distribution requires.",
}, note="The obligation people are likeliest to overlook, and the one with the "
        "sharpest teeth in several jurisdictions. Kept separate from the "
        "general responsibility clause so it cannot be skimmed past.")
add("licence.halation.liability", {
    "en": "Halation is provided as is, without warranty of any kind. To the "
          ...
          "produced with it.",
    "zh-Hant": "Halation 按「現狀」提供，不附任何形式之保證。在法律允許之最大範圍內，其作者對因使用本軟體或因以本軟體製作之媒體所生之任何損失、損害或請求，概不承擔責任。",
    "zh-Hans": "Halation 按“现状”提供，不附任何形式之保证。在法律允许之最大范围内，其作者对因使用本软件或因以本软件制作之媒体所生之任何损失、损害或请求，概不承担责任。",
    "de": "Halation wird ohne Mängelgewähr und ohne jede Garantie bereitgestellt. "
          "Soweit gesetzlich zulässig, übernehmen die Autoren keine Haftung für "
          "Verluste, Schäden oder Ansprüche, die aus der Nutzung oder aus den "
          "damit erzeugten Medien entstehen.",
    "ar": "يُقدَّم Halation كما هو، دون أي ضمان من أي نوع. وإلى أقصى حد يسمح به "
          "القانون، لا يتحمّل مؤلفوه أي مسؤولية عن خسارة أو ضرر أو مطالبة تنشأ "
          "عن استخدامه أو عن الوسائط المنتجة به.",
    "ja": "Halation は現状有姿で提供され、いかなる種類の保証も伴いません。法令が許す最大限の範囲において、その作者は、本ソフトウェアの利用または本ソフトウェアで作成された媒体に起因する損失・損害・請求について一切の責任を負いません。",
    "ko": "Halation은 있는 그대로 제공되며 어떠한 보증도 하지 않습니다. 법이 허용하는 최대 범위에서, 제작자는 본 소프트웨어의 사용이나 이를 통해 만들어진 미디어로 인한 손실·손해·청구에 대하여 어떠한 책임도 지지 않습니다.",
    "th": "Halation จัดให้ตามสภาพที่เป็นอยู่ โดยไม่มีการรับประกันใด ๆ "
          "ภายในขอบเขตสูงสุดที่กฎหมายอนุญาต ผู้เขียนไม่รับผิดต่อความสูญเสีย ความเสียหาย "
          "หรือข้อเรียกร้องใดอันเกิดจากการใช้งานหรือจากสื่อที่ผลิตด้วยเครื่องมือนี้",
    "yue-Hant": "Halation 按「現狀」提供，唔附任何形式嘅保證。喺法律容許嘅最大範圍內，作者對因使用本軟件或者用佢整出嚟嘅媒體而引致嘅任何損失、損害或者申索，一概唔負責。",
    "en-SG": "Halation is provided as is, without warranty of any kind. To the "
             "fullest extent permitted by law, its authors accept no liability "
             "for any loss, damage or claim arising from its use or from the "
             "media produced with it.",
})
add("licence.halation.acceptance", {
    "en": "Using Halation constitutes acceptance of these terms and of the "
          "licences of every model you choose to run.",
    "zh-Hant": "使用 Halation 即表示您接受本條款，以及您所選擇執行之每一模型之授權條款。",
    "zh-Hans": "使用 Halation 即表示您接受本条款，以及您所选择运行之每一模型之授权条款。",
    "de": "Die Nutzung von Halation gilt als Annahme dieser Bedingungen sowie "
          "der Lizenzen jedes Modells, das Sie ausführen.",
    "ar": "إن استخدام Halation يُعدّ قبولًا لهذه الشروط ولتراخيص كل نموذج تختار "
          "تشغيله.",
    "ja": "Halation を利用することは、本条件および利用者が実行を選択したすべてのモデルのライセンスに同意したものとみなされます。",
    "ko": "Halation을 사용하는 것은 본 약관과 이용자가 실행하기로 선택한 모든 모델의 라이선스에 동의하는 것으로 간주됩니다.",
    "th": "การใช้ Halation ถือเป็นการยอมรับข้อกำหนดเหล่านี้ "
          "และสัญญาอนุญาตของทุกโมเดลที่คุณเลือกเรียกใช้",
    "yue-Hant": "使用 Halation 即代表你接受本條款，以及你揀嚟行嘅每一個模型嘅授權條款。",
    "en-SG": "Using Halation constitutes acceptance of these terms and of the "
             "licences of every model you choose to run.",
})
add("problem.licence", {
    "en": "The model licence has not been accepted yet.",
    "zh-Hant": "尚未接受模型授權條款。", "zh-Hans": "尚未接受模型授权条款。",
    "de": "Die Modelllizenz wurde noch nicht akzeptiert.",
    "ar": "لم يُقبل ترخيص النموذج بعد.", "ja": "モデルのライセンスにまだ同意していません。",
    "ko": "모델 라이선스에 아직 동의하지 않았습니다.",
    "th": "ยังไม่ได้ยอมรับสัญญาอนุญาตของโมเดล",
    "yue-Hant": "仲未接受模型授權條款。",
    "en-SG": "The model licence has not been accepted yet.",
}, note="A blocking problem on Compose, beside a button that opens the terms. "
        "The welcome dialog can be skipped, so this is reachable in normal use.")
add("summary.reviewLicence", {
    "en": "Review licence", "zh-Hant": "查看授權條款", "zh-Hans": "查看授权条款",
    "de": "Lizenz ansehen", "ar": "مراجعة الترخيص", "ja": "ライセンスを確認",
    "ko": "라이선스 검토", "th": "ดูสัญญาอนุญาต", "yue-Hant": "睇下授權條款",
    "en-SG": "Review licence",
}, note="Opens the terms as a sheet from Compose, for someone who skipped the "
        "welcome dialog. 'Review' rather than 'Accept': the button opens the "
        "text, it does not agree on anyone's behalf.")
